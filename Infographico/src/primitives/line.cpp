...
bool Line::isSelected(const float& reqX, const float& reqY) const {
    bool value = false;
    if (x2 != x1) {
        if (x1 - max(thickness, 5.0f) <= reqX &&
                reqX <= x2 + max(thickness, 5.0f) &&
                y1 - max(thickness, 5.0f) <= reqY &&
                reqY <= y2 + max(thickness, 5.0f)) {
            float gradient = (y2 - y1) / (x2 - x1);
            float intercept = y2 - gradient*x2;
            if (abs(gradient) < 1.0f) {
                if (abs(reqY - (gradient*reqX + intercept)) < max(thickness, 5.0f)) {
                    value = true;
                }
            } else if (abs(reqX - (reqY - intercept)/gradient) < max(thickness, 5.0f)) {
                value = true;
            }
        }
    } else if (x1 - reqX < max(thickness, 5.0f) &&
            y1 <= reqY &&
            reqY <= y2) {
        value = true;
    }
